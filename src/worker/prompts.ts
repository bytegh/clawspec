...
export function buildPlanningPrependContext(params: {
  ...
  const workflow = params.mode === "sync"
    ? [
        ...
        "5. Use the current visible chat context plus the planning journal to gather requirements, but structure them according to the instruction prompts.",
        "6. Do not invent endpoints, features, constraints, files, acceptance criteria, test scenarios, or architecture details that are not grounded in the planning journal or user requirements.",
